...
namespace ChristianHelle.DatabaseTools.SqlCe
{
    public class SqlCeDatabase : ISqlCeDatabase
    {
        ...
        private static Dictionary<string, Table> GetTableInformation(ICollection<string> tables, DataSet schema)
        {
            ...
            foreach (var tableName in tables)
            {
                ...
                foreach (DataRow row in columns.Rows)
                {
                    ...
                    column.ManagedType = GetManagedType(row.Field<string>("DATA_TYPE"), column.AllowsNull);
        ...
        private static Type GetManagedType(string dataType, bool allowsNull)
        {
            switch (dataType)
            {
                case "bit":
                    return allowsNull ? typeof(bool?) : typeof(bool);
                case "varbinary":
                case "image":
                    return typeof(byte[]);
                case "tinyint":
                    return typeof(byte);
                case "datetime":
                    return allowsNull ? typeof(DateTime?) : typeof(DateTime);
                case "numeric":
                    return allowsNull ? typeof(decimal?) : typeof(decimal);
                case "float":
                    return allowsNull ? typeof(double?) : typeof(double);
                case "uniqueidentifier":
                    return allowsNull ? typeof(Guid?) : typeof(Guid);
                case "smallint":
                    return allowsNull ? typeof(short?) : typeof(short);
                case "int":
                case "integer":
                    return allowsNull ? typeof(int?) : typeof(int);
                case "uint32":
                    return allowsNull ? typeof(uint?) : typeof(uint);
                case "bigint":
                    return allowsNull ? typeof(long?) : typeof(long);
                case "uint64":
                    return allowsNull ? typeof(UInt64?) : typeof(UInt64);
                case "char":
                case "nchar":
                case "text":
                case "ntext":
